# NOVAX TRAVEL — Single Source of Truth (SSoT) — Enterprise MAX Edition v3.1 (EN)
**Date:** 2025-12-21  
**Status:** Canonical / Governing Specification  
**Strict rule:** This document is the single governing specification. If any repo/document conflicts with this SSoT, **this SSoT wins**.

---

## 0. Executive Summary
NOVAX TRAVEL is a multi-channel travel platform (Customer Web, Admin Web, Mobile App) powered by an API-first backend with enterprise-grade security, observability, and operational readiness. This SSoT defines system boundaries, providers, repository structure, deployment topology, security baseline, API/data standards, CI/CD, and “Definition of Done” gates that must be satisfied before declaring anything complete.

---

## 1. Non‑Negotiables (Hard Laws)
1) **PR‑only / Main protected** — No direct pushes to `main`.  
2) **No secrets in code** — No keys, DB URLs with passwords, tokens, private certs, etc. in repositories or ZIPs. Secrets live only in secret managers (GitHub/Render/Vercel/Neon/Cloudflare).  
3) **Evidence‑based completion** — No “done” without verifiable evidence (URLs, steps, screenshots, CI logs).  
4) **Multi‑repo boundaries** — Backend/Web/Admin/Mobile/Infra cannot be mixed.  
5) **Fixed platform providers** — Render (backend), Vercel (web/admin), Neon (PostgreSQL), Cloudflare (DNS/SSL/WAF), Backblaze B2 (storage), Firebase FCM (push).  
6) **Mandatory subdomains** — `novaxtravel.com`, `www.novaxtravel.com`, `api.novaxtravel.com`, `admin.novaxtravel.com`.  
7) **Health gate** — `GET https://api.novaxtravel.com/health` must return OK and be used for uptime monitoring.  
8) **Security baseline mandatory** — WAF + Rate limits + Audit logs + Backups are mandatory from day 1.  
9) **Owner‑friendly verification** — Every release must include steps a non‑technical owner can follow to validate readiness.

---

## 2. Canonical Repo Map (Enterprise Multi‑Repo)
> Repo names may vary (e.g., `novax-backend` vs `backend-api`), but **responsibilities are immutable**.

### 2.1 Repositories
1) **backend-api** (Laravel)  
   - Auth, RBAC, Catalog, Search/Pricing rules, Booking lifecycle, Wallet/Loyalty, Agents/B2B, Finance, Notifications, Logs/Audit, Seeds.
2) **web-portal** (Next.js — Customer)  
   - Search, booking flows, user/profile, wallet/loyalty, guest mode, payments UX.
3) **admin-panel** (Next.js — Ops/Admin)  
   - CRUD operations, approvals, RBAC management, finance ops, agent/company management, datasets, auditing, BI entry points.
4) **mobile-app** (Flutter — Android first)  
   - Offline-first UX, local encrypted storage, sync engine, push notifications, OTA config.
5) **infrastructure-config** (Infra & platform configuration)  
   - DNS mapping, environment templates, deployment runbooks, CI templates, diagrams, evidence master.
6) **docs**  
   - This SSoT, architecture, OpenAPI, RBAC matrix, data overview, checklists, evidence package, ADRs, runbooks.

### 2.2 Hard boundary rule
- No repo may contain another repo’s runnable application.  
- Shared contracts (OpenAPI, RBAC, design tokens, seed datasets) are stored in **docs** and imported/replicated with explicit versioning.

---

## 3. Environments, Domains, and Traffic Flow
### 3.1 Environments
- **Production** (mandatory)  
- **Staging** (strongly recommended; treat as required if available)

### 3.2 Domains (Production)
- `novaxtravel.com` → Customer web (Vercel)
- `www.novaxtravel.com` → Customer web (Vercel)
- `admin.novaxtravel.com` → Admin web (Vercel)
- `api.novaxtravel.com` → Backend API (Render)

### 3.3 Traffic & security
Client → Cloudflare (WAF/Rate limits/SSL) → Origin (Vercel/Render).  
SSL mode: **Full (Strict)**.

---

## 4. System Architecture (Reference Model)
### 4.1 High-level diagram (text)
```
[Customer Web (Vercel)] ----[Admin Web (Vercel)] -------->  [Cloudflare]  --->  [Backend API (Render)]  ---> [Neon PostgreSQL]
[Mobile App (Android)] ----/                         |       |
                                                     |       +--> [Backblaze B2 (S3)]
                                                     +--> [FCM Push] / [Email/SMS Provider]
```

### 4.2 Architecture principles
- API-first, contract-driven (OpenAPI).  
- Least privilege access; RBAC enforced server-side.  
- Observability-first (structured logs, correlation IDs, audit trail).  
- Idempotent payments and booking operations.  
- Offline-first mobile with safe sync semantics.

---

## 5. Functional Domains (Enterprise Coverage)
1) Identity & Access (Auth/RBAC/Users/Devices/Agents/B2B)  
2) Catalog & Content (Flights/Hotels/Cars/Visas + managed content)  
3) Search, Pricing & Rules Engine (markups, commissions, FX, policies)  
4) Booking Lifecycle (create/reserve/pay/issue/modify/cancel/refund)  
5) Wallet & Loyalty (earn/burn, ledgered transactions)  
6) Finance (invoices, settlements, refunds, exports, approvals)  
7) Notifications (email/sms/push templates, retries)  
8) Administration & Operations (CRUD, approvals, audit, reporting, config)  

---

## 6. API Contract Standard (Mandatory)
### 6.1 Versioning
- All production APIs under `/v1/...`. Breaking changes require new version.
...
### 6.4 Idempotency (payments/bookings/refunds)
- Require `Idempotency-Key` on create booking / payment / refund.
- Persist key and replay identical response for duplicates within retention window.

### 6.5 OpenAPI governance
- Canonical OpenAPI spec in `docs/openapi/openapi.yaml`.
- Any API change must update OpenAPI + changelog + tests.

---

## 7. Data Standard (PostgreSQL on Neon)
### 7.1 Data modeling rules
- Choose a single PK standard (UUID recommended) and apply everywhere.
- Timestamps: `created_at`, `updated_at`; `deleted_at` for soft deletes as needed.
- Enforce FKs and indices; avoid orphan rows.
- Monetary model: store amount + currency code; prefer minor units or explicit decimal standard consistently.

### 7.2 Data classification
- Public / Internal / Sensitive / Restricted.

### 7.3 Audit fields
- Capture actor/context fields for sensitive actions.

---

## 8. RBAC (Enterprise Baseline)
### 8.1 Principles
- Least privilege; separation of duties.
- Deny-by-default; explicit permissions.

### 8.2 Required deliverable
`docs/security/RBAC_MATRIX.md` must enumerate:
- Roles × Modules × Actions (view/create/update/delete/approve/export)

### 8.3 Dual control
- Refunds, wallet adjustments, pricing rules, RBAC changes.

---

## 9. Security Baseline (Mandatory)
- Cloudflare WAF + Rate limits
- Secret management policy (no secrets in code)
- Audit logs for all sensitive actions
- Vulnerability scanning and blocking critical findings

---

## 10. Observability & Operations (SRE‑ready)
- `/health` mandatory; uptime checks configured.
- Metrics and alerting baseline defined.
- Backup & DR documented with rollback plans.

---

## 11. CI/CD (Enterprise Minimum)
- CI required on PRs; lint + test + build as required checks.
- Backend deploy to Render; web/admin deploy to Vercel; mobile artifacts built.

---

## 12. Deployment Runbook (Provider‑specific)
- Cloudflare DNS + SSL Full (Strict) + WAF/rate limits
- Vercel projects: web + admin
- Render service: backend with `/health`
- Neon DB: least-privilege user, controlled migrations, deterministic seeding

---

## 13. Documentation Set (Mandatory in GitHub)
### 13.1 In every repo root
- `README.md`, `SECURITY.md`, `DEPLOYMENT.md`, `CHECKLIST.md`, `EVIDENCE_PACKAGE.md`, `CODEOWNERS`, `.github/pull_request_template.md`

### 13.2 In docs repo
- `architecture/`, `openapi/openapi.yaml`, `security/RBAC_MATRIX.md`, `data/`, `ops/`, `evidence/`, `adr/`

---

## 14. Definition of Done (Gates) + Evidence
A release is “done” only when evidence confirms:
1) `https://novaxtravel.com` loads correctly.
2) `https://www.novaxtravel.com` loads correctly.
3) `https://api.novaxtravel.com/health` returns OK.
4) `https://admin.novaxtravel.com` loads and login works.
5) SSL lock present; Cloudflare Full (Strict).
6) CI green on all relevant repos.

---

## 15. Yemen Seed Data (Mandatory)
Ship canonical Yemen datasets (governorates, cities, airlines, routes).  
Store canonical datasets in docs repo and provide import scripts in backend.

---

# ENTERPRISE MAX EXTENSIONS (v3.1 Additions)

## 16. Threat Model (Minimum Mandatory)
Deliverable: `docs/security/THREAT_MODEL.md` covering assets, threats, mitigations.

## 17. SLO/SLA (Operational Targets)
Deliverable: `docs/ops/SLO_SLA.md` (availability, latency, error budgets).

## 18. Testing Strategy (Enterprise)
Deliverable: `docs/qa/TEST_STRATEGY.md` (unit/integration/contract/smoke).

## 19. Release & Change Management
Deliverable: `docs/ops/RELEASE_PROCESS.md` (versioning, approvals, rollback).

## 20. Architecture Decision Records (ADRs)
Deliverable: `docs/adr/0000-template.md` + ADR log.

## 21. Risk Register (Mandatory)
Deliverable: `docs/ops/RISK_REGISTER.md`.

## 22. Data Retention & Privacy (Baseline)
Deliverable: `docs/security/DATA_RETENTION_PRIVACY.md`.

## 23. Secure SDLC (Enterprise)
Deliverable: `docs/security/SECURE_SDLC.md`.

## 24. Evidence Master (Owner-Friendly)
Deliverable: `docs/evidence/EVIDENCE_MASTER.md`.

## 25. Implementation Agent Instruction Block (Final)
> You are the implementation agent. Your only specification is `NOVAX_TRAVEL_SSoT_v3.1_Enterprise_EN_Max.md`.  
> Read the entire document and Appendix A first, enforce PR-only, never commit secrets, and produce evidence for the gates.

---

# Appendix A — Original SOURCE OF TRUTH (Verbatim / No Loss)
```markdown
